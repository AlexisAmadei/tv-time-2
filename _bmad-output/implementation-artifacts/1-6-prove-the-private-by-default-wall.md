...
- [x] Task 3: Cross-user RLS wall proof — durable check in `scripts/smoke-check.mjs` (AC: #2, #4, #7)
  ...
  - [x] Use two fixed, clearly-named smoke accounts so reruns are idempotent and never accumulate junk users: `smoke-test-a@tv-time-2.invalid` / `smoke-test-b@tv-time-2.invalid`, usernames `smoketestusera` / `smoketestuserb`, a shared fixed password. (`.invalid` is the RFC 2606 reserved TLD for exactly this "will never be a real deliverable address" case — appropriate for a synthetic identity that must never accidentally match a real signup.)
...
- **Idempotency preserved, not cleanup**: the two smoke-test accounts (`smoke-test-a@tv-time-2.invalid` / `smoke-test-b@tv-time-2.invalid`) and their fixed-id watch row are never deleted — reruns sign in (not sign up) and upsert the same row, verified by running `pnpm run verify` three times in a row with identical green output.
