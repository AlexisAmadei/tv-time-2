...
- [Source: _bmad-output/planning-artifacts/architecture/architecture-tv-time-2-2026-07-02/ARCHITECTURE-SPINE.md#AD-4] — "Watch commits, and the rating/mood/note that follows, are local-first via one durable outbox unit" — the contract this story's `note` widening slots into verbatim; AD-4's own text already lists `note` as a first-class member of the reaction, not an afterthought.
- [Source: _bmad-output/planning-artifacts/architecture/architecture-tv-time-2-2026-07-02/ARCHITECTURE-SPINE.md#AD-3] — watch is the atomic timestamped unit; a note edit updates *that row*, never collapsing across separate watches (AC3).
